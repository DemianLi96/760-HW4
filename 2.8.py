...
def main():
    ...
    filename = './languageID/e11.txt'
    e11_characters = []
    e11_characters = get_file_characters(filename,e11_characters)

    e11_char_count = []
    for cha in chars:
        e11_char_count.append(e11_characters.count(cha))
    e11_char_count = np.array(e11_char_count)
    # print(e11_char_count)
    # print(e_vector_log)
    e_pred_log = np.dot(e_vector_log.T,e11_char_count)+np.log(pye)
    j_pred_log = np.dot(j_vector_log.T,e11_char_count)+np.log(pyj)
    s_pred_log = np.dot(s_vector_log.T,e11_char_count)+np.log(pys)
    predict_language = languages[np.argmax(np.array([e_pred_log, j_pred_log, s_pred_log]))]
    # print(e_pred_log)
    # print(j_pred_log)
    # print(s_pred_log)
    print('When e11.txt is not arbitrarily shuffled:')
    ...
    random.shuffle(e11_characters)
    e11_char_count2 = []
    for cha in chars:
        e11_char_count2.append(e11_characters.count(cha))
    e11_char_count2 = np.array(e11_char_count2)
    e_pred_log = np.dot(e_vector_log.T, e11_char_count2) + np.log(pye)
    j_pred_log = np.dot(j_vector_log.T, e11_char_count2) + np.log(pyj)
    s_pred_log = np.dot(s_vector_log.T, e11_char_count2) + np.log(pys)
    print('When e11.txt is arbitrarily shuffled:')
    print('p\u0302(y = e | x) = exp(' + str(e_pred_log) + ')/p(x)')
    print('p\u0302(y = j | x) = exp(' + str(j_pred_log) + ')/p(x)')
    print('p\u0302(y = s | x) = exp(' + str(s_pred_log) + ')/p(x)')
    print('The predict language is', predict_language)
